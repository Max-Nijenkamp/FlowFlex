...
- **Toast pile-up ruins screenshots**: every Livewire action stacks a success notification top-right; after seeding data via UI clicks (e.g. quick-adding board deals) the stack covers the content. `page.reload()` once before the shot, or seed via tinker instead.
- **Screenshot in the state the owner complained about**: "can't see my deals" needs a column with 10 deals, "dark mode broken" needs `localStorage.setItem('theme','dark')` + reload — an empty light-mode screenshot proves nothing. Programmatic theme flip beats clicking the sidebar toggle.
