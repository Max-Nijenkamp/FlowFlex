    ...
    <div class="grid gap-4" style="grid-template-columns: repeat({{ max($this->getStages()->count(), 1) }}, minmax(16rem, 1fr)); overflow-x: auto;">
        @forelse ($this->getStages() as $stage)
            <div class="rounded-xl bg-gray-50 dark:bg-gray-800/50 p-3"
                 x-data
                 @dragover.prevent
                 @drop.prevent="$wire.moveDeal($event.dataTransfer.getData('deal'), '{{ $stage->id }}')">
                ...
                <div class="space-y-2 min-h-16">
                    @foreach ($this->getDealsFor($stage->id) as $deal)
                        <div class="rounded-lg bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 p-3 cursor-grab active:cursor-grabbing"
                             draggable="true"
                             x-data
