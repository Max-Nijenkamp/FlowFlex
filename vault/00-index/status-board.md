...
| Date | Scope | Work |
|---|---|---|
| 2026-07-05 | public site round 3 | Owner feedback: header nav "not FlowFlex — should show and explain, not sell". Rebuilt per SaaS mega-menu practice (educate + describe, group by visitor problem, ≤7 top-level items, accordion on mobile): **Product dropdown** = Departments column (4 domain rows w/ color squares + one-liners → /product/{key}) + How-it-works column (/product, /switch, /calculator) + ink mono foot strip "73 modules · one database · 0 integrations → All modules"; **Resources dropdown** = Help/Customer story/Trust/Changelog/Status w/ one-liners. Hover-intent open (160ms leave delay), click toggle, Esc + outside-click close, aria-haspopup/expanded; mobile menu regrouped w/ mono section labels + domain squares. Verified: hover/click/Esc/navigation e2e, desktop + 390px screenshots, zero page errors. |
| 2026-07-05 | public site round 2 | Owner feedback: "too AI-like/plain, missing pages". **Character pass** — the Switchboard+ kinetic signatures landed: animated flow-pulse SVG rails behind the hero switchboard (data streaming into the board, indigo/sky, staggered), flow-band node dots now breathe (staggered glow keyframes), richer 404 copy, "See the modules" CTAs → /modules. **All 8 handoff extras BUILT** (extras-public.jsx + extras2-public.jsx): /modules (catalogue w/ working domain filters), /switch (stack-translation map + 3-step plan + migration flow band), /trust (6 guarantee cells + procurement FAQ), /changelog (timeline rail w/ domain dots), /calculator (patchwork-tax: editable tool repeater + team slider + reactive side-by-side receipt — verified totals recompute on row remove), /customers/veldkamp-logistics (case study: result cells, month rail, sticky pull-quote), /help (live search filter over articles), /status (uptime bars + incident, server-stamped checked-at). Footer regrouped: Product/Company/Legal&Trust columns carry all 18 pages. All screenshot-verified, zero page errors, Pint clean. |
