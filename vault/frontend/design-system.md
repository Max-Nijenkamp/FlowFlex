...
**Panel chrome (§12, implemented)**: sidebar footer = workspace switcher (mono "Your workspaces" label + trigger card showing current-domain square + name, opens the panel-selection modal; replaced the "Your panels" chips 2026-07-05, `SIDEBAR_FOOTER` hook) + user card (avatar initial, name, company) which IS the account menu — clicking it opens an upward popover with a light/dark/system theme row, Profile + Sign out (owner decision 2026-07-04: topbar user menu hidden entirely, no avatar in the topbar); topbar = 320px search trigger with ⌘K kbd (opens Spotlight via `ff-spotlight-open` window event, `GLOBAL_SEARCH_BEFORE` hook) + 34px bordered bell, NOTHING else (owner decision 2026-07-04: no topbar crumb — breadcrumbs render on the page header instead, styled faint 13px; sidebar collapse toggle lives in the sidebar footer, not the topbar). Panel switching lives ONLY in the footer workspace switcher — the profile dropdown carries no "Switch to" items (PanelSwitchItems removed 2026-06-12; chips folded into the switcher 2026-07-05). Switcher modal: domain-palette hairline top strip, mono count sub, 30px tinted domain tiles (square inside, 3px radius), per-row domain-color hover/current tints via `--ws-c`, slide-in chevrons, owner foot strip → marketplace. Panel→panel navigation: `.fi-main-ctn` entrance fade (`ff-page-in` 0.22s ease-out) — never `@view-transition` (freezes rendering after Livewire redirects, panel-chrome gotcha §6.12).
