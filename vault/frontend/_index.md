...
> **Build status (2026-07-05 — SITE LIVE, round 2)**: Rebuilt greenfield after the app reset, from `design_handoff_flowflex_site 2/`. Shipped: Home (hero flow-pulse rails — the kinetic signature), Pricing (reactive calculator + sticky receipt), Product, Domain detail (`/product/{hr|finance|crm|projects}`, one dynamic page), About, Contact (useForm + honeypot + throttled controller + queued `ContactMessageMail`), Terms, Privacy, branded 404 fallback, **plus the §14–24 extras**: `/modules` (filterable catalogue), `/switch`, `/trust`, `/changelog`, `/calculator` (patchwork-tax repeater, reactive), `/customers/veldkamp-logistics`, `/help` (live search), `/status` (static uptime mock — spatie/laravel-health wiring still open per §23). `MarketingLayout` + FlowBand/CtaBand/ReplacesStrip/FfSwitch/Logo/Reveal in `Components/`; content data in `resources/js/data/marketing.ts` + `productContent.ts`; ff.css ported into `app.css` under `.ff` scope (class names preserved). **Deviations**: `/login` redirects to `/app/login` (panel login is the designed screen; Vue auth pages deferred), Ziggy installed but pages use literal hrefs, no Pinia (not needed yet), no blog/portals (Phase 2 unchanged). Fonts via bunny.net (GDPR-safe).
