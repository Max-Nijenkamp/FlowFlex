...
### Unit
- [x] `FlowFlexMailable` resolves name / logo / primary colour from the current `CompanyContext`

### Feature (Pest)
- [x] Rendered mail contains the sending company's branding (`MailBrandingTest`)
- [x] Mailable is queued on `notifications`, never sent synchronously
- [x] A queued mail restores the correct tenant via `WithCompanyContext` — no cross-company branding
- [x] An address with `email_deliverable = false` is skipped
