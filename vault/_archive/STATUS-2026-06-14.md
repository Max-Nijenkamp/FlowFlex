...
|---|---|---|---|
| 2026-06-19 | All | strip to App+Admin shell | Full domain wipe (HR/Finance/CRM): 470 files deleted — Filament panels + PanelProviders + domain ServiceProviders, Models/Services/Contracts/Data/Events/States/Actions/Listeners/Exceptions, 13 domain migrations, factories, domain tests, 5 Api/V1 controllers + Careers/QuoteAccept public controllers. Kept: public Switchboard+ frontend, auth/login, `/app` + `/admin` panels, all platform models. Rewired bootstrap/providers, api+web routes, LocalDevSeeder (keeps test@test admin + Demo tenant/owner, no domain demo data), PermissionSeeder (core perms only), SidebarFooter (APP chip only), LayersTest+PanelAuthTest (repointed to /app), phpstan.neon (stale Finance ignores removed). `config/flowflex.php` modules KEPT (marketing-site catalog, no code link). ADR [[build/decisions/decision-2026-06-19-strip-to-app-admin-shell]]. Gates: 186 Pest pass, Pint clean, PHPStan 0, migrate:fresh --seed clean. Branch chore/strip-to-app-admin-shell |
